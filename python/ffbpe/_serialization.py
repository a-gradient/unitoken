...
MODEL_CONFIG_FILENAME = "ffbpe.json"
LEGACY_MODEL_CONFIG_FILENAME = "unitoken.json"
...
def read_model_config(directory: Path) -> ModelConfig:
  ...
  if not config_path.exists():
    legacy_config_path = directory / LEGACY_MODEL_CONFIG_FILENAME
    if legacy_config_path.exists():
      config_path = legacy_config_path
  try:
    value = json.loads(config_path.read_text(encoding="utf-8"))
  except (OSError, json.JSONDecodeError) as error:
    raise ValueError(f"Cannot read FFBPE model config at {config_path}: {error}") from error

  if not isinstance(value, dict):
    raise ValueError(f"Invalid FFBPE model config at {config_path}: expected an object")
  if value.get("version") != MODEL_CONFIG_VERSION:
    raise ValueError(
      f"Unsupported FFBPE model config version {value.get('version')!r}; "
  ...
  for field, field_type in required.items():
    if not isinstance(value.get(field), field_type):
      raise ValueError(f"Invalid FFBPE model config field {field!r}")
  if value["unit"] not in ("byte", "unicode"):
    raise ValueError(f"Invalid FFBPE model unit {value['unit']!r}")
  if value["format"] not in ("gpt2", "unitoken"):
    raise ValueError(f"Invalid FFBPE model format {value['format']!r}")
  for field in ("vocab_file", "merges_file"):
    model_path = Path(value[field])
    if model_path.is_absolute() or ".." in model_path.parts:
      raise ValueError(f"Invalid FFBPE model config field {field!r}: expected a relative path")
  if not all(isinstance(token, str) for token in value["special_tokens"]):
    raise ValueError("Invalid FFBPE model config field 'special_tokens'")
  if value.get("pat_str") is not None and not isinstance(value["pat_str"], str):
    raise ValueError("Invalid FFBPE model config field 'pat_str'")
  if value.get("unicode_bigrams") is not None and (
    ...
    raise ValueError("Invalid FFBPE model config field 'unicode_bigrams'")
  if value["unicode_bigram_mixed_boundary"] not in ("keep", "split"):
    raise ValueError("Invalid FFBPE model config field 'unicode_bigram_mixed_boundary'")
