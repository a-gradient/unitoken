...
impl BpeModel<u8, Idx> {
  ...
  pub fn to_encoder(&self) -> MyResult<BpeEncoder<u8>> {
    self.to_encoder_with_options(None, true)
  }

  /// Build an encoder with an optional pre-tokenizer pattern and vocabulary-bigram splitting policy.
  pub fn to_encoder_with_options(
    &self,
    pat_str: Option<&str>,
    split_on_vocab_bigrams: bool,
  ) -> MyResult<BpeEncoder<u8>> {
